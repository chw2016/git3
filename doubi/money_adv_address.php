...
<title>限时秒杀</title>
<link rel="stylesheet" type="text/css" href="./money_css/phone-reset.css">
<script type="text/javascript" src="./money_js/jquery-1.9.1.min.js"></script>
<script type="text/javascript" src="./money_js/regionv3.js"></script>
<script type="text/javascript" src="./money_js/81"></script>
<style type="text/css">
body{
...
<body class="ios">
	...
	<div class="wrapper">
		...
		<a id="btn-buy" class="btn-buy" href="javascript:void(0)">在线支付</a>
		<a id="btn-buys" class="btn-online" href="javascript:void(0)">货到付款</a>
	</div>
	<script type="text/javascript">
	$(function(){
		$('.btn-buy').click(function(){
			...
			if (typeof WeixinJSBridge == "undefined"){
				...
				$.ajax({
					type: 'POST',
					url: 'money_adv_jsApiParameters.php?action=weixin_pay',
			...
			WeixinJSBridge.invoke(
				...
				function(res){
					if(res.err_msg == "get_brand_wcpay_request:ok" ){
						$.ajax({
							type: 'POST',
							url: 'money_adv_jsApiParameters.php?action=weixin_update',
							...
							success: function(data){
								alert('支付成功');
								document.location.href="money_adv_order.php?openid="+uopenid+"&id="+proid+'&sprice='+sprice;
							},
							error: function(xhr, type){
								alert('支付异常，请重新充值');
							}
		...
		$('.btn-online').click(function(){
			...
			$('.btn-online').html('购买中...');
			$.post("money_adv_shopping_ajax.php?action=address",{"proid":proid,"address":address,"province":province_str,"city":city_str,"district_str":district_str,"mobile":mobile,"name":name},function(data){
				if(data == 1){
					alert('购买成功');
					document.location.href="money_adv_order.php?openid="+uopenid+"&id="+proid+"&sprice=20";
				}else{
					alert('购买失败');
				}
