...
<div class="return-top">
    ...
    <span class="return-text">修改类似优惠信息</span>
</div>
...
<section class="ucenter-main animated fadeInDown">
    ...
    <ul class="um-list um-list-form">
        <li>
            <label for="cuser_name" class="label">卡券名</label><input class="title" type="text" maxlength="9" value="<?php echo $merchant_id['title'];?>">
        </li>
        <li><label for="cuser_name" class="label">商户名称</label><input type="text" class="brand_name" value="<?php echo $merchant_id['brand_name'];?>"></li>
        <li>
            <label for="cuser_name" class="label">券名</label><input type="text" class="sub_title"  value="<?php echo $merchant_id['sub_title'];?>">
        </li>
        <li>
            <label for="cuser_name" class="label">单次领取数</label><input type="text"  value="<?php echo $merchant_id['per_num'];?>" class="per_num" onkeyup="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))">
        </li>
        <li>
            <label for="cuser_name" class="label">领取的数量</label><input type="text"  value="<?php echo $merchant_id['quantity'];?>" class="quantity" onkeyup="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))">
        </li>
        <li>
            <label for="cuser_name" class="label">逗币总量</label><input type="text" class="total_doubi" readonly="readonly" value="<?php echo $merchant_id['quantity']*$merchant_id['per_num'];?>">
        </li>
        <li>
            <label for="cuser_name" class="label" style="width: 22%;">开始时间</label> <input type="text" placeholder="开始时间" id="start_time" class="date" value="<?php echo $merchant_id['begin_timestamp'];?>">
        </li>
        <li>
            <label for="cuser_name" class="label" style="width: 22%;">结束时间</label> <input type="text" placeholder="结束时间" id="end_time" class="datetime" value="<?php echo $merchant_id['end_timestamp'];?>">
        </li>
    </ul>
</section>
<header class="ucenter-t" style="margin-top: 30px;">
...
<section class="ucenter-main animated fadeInDown">
    ...
    <ul class="um-list um-list-form" style="    height: 100px;">
        <li><textarea class="description" rows="6" cols="35" placeholder="" ><?php echo $merchant_id['description'];?></textarea></li>
    </ul>
</section>
...
<script type="text/javascript">
    ...
    Zepto(function($){
        ...
        $('.save_update').tap(function(){
            ...
            $.post("money_query_device_group.php?action=likes",DATA,function(reg){
                if (reg.msg == 1) {
                    var DG = $.dialog({
                        content: '配置成功',
                        button: ['ok']
