...
$.fn.ProvinceCity = function(){
	var _self = this;
	//定义3个默认值
	_self.data("province",["省份", "省份"]);
	_self.data("city1",["城市", "城市"]);
	_self.data("city2",["区县", "区县"]);
	//插入3个空的下拉框
	_self.append("<select name='seng' id='seng'></select>");
	_self.append("<select name='si' id='si'></select>");
	_self.append("<select name='xian' id='xian'></select>");
	//分别获取3个下拉框
