...
* @Last Modified by:   星辉
* @Last Modified time: 2015-03-18 15:32:55
*/

$(function(){

	// 初始化用户购物车物品的数量
	var storage = localStorage["CartJson_"+goodsshopid];
	if(storage){
	  storage = JSON.parse(localStorage["CartJson_"+goodsshopid]);
	$.each(storage.goods_list, function(i, obj){
		$('.countBuy.'+i).html(obj.count);
		$('.countBuy.'+i).siblings('.circle').data('count', obj.count)
	})
	}
				...
				if(localStorage['CartList_'+goodsshopid]){
					$(".Cartlist").html(localStorage['CartList_'+goodsshopid]);
				...
				$(".addOne").on('touchstart',function(e) {
					...
					$(this).on("touchend",function(){
						if(!ISMove){
                            ...
                            var des = GoodsData[goodsId].des;				//通过键值获取该商品的简介
                            var pic	 = GoodsData[goodsId].pic;						//通过键值获取该商品的图片
//alert(price);alert(goodsname)
					...
					THIS.parent().find(".countBuy").fadeIn(50).html(count);   			//显示该商品的添加数量
					...
					localStorage['CartJson_'+goodsshopid]=JSON.stringify(Cart);
					localStorage['CartList_'+goodsshopid]=$(".Cartlist").html();
					//console.log(localStorage.CartList);
						}
					});
				});
				...
				$(".deleteone").on('touchend', function(e) {
                    ...
                    var shopname = goods_list[goodsId].shopname;					//通过键值获取该商店的名称
                    var shopId	 = goods_list[goodsId].shopId;						//通过键值获取该商店的id
                    var des      = goods_list[goodsId].des;						//通过键值获取该商品的简介
                    var pic	 = GoodsData[goodsId].pic;						//通过键值获取该商品的图片
				//alert(price);alert(goodsname)
					if(THIS.hasClass('wares')){
							...
							if(count>0){
								...
								goods.each(function(i,o){
						     		if($(o).attr("goodsId")==goodsId){
						     				$(o).data('count', count);
						     				$(o).parent().find('.countBuy').html(count);
								...
								if(count==0){
									...
									$("#delete_"+goodsId).addClass('hide');	
									$(".G"+goodsId).slideUp(200,function(){$(".G"+goodsId).remove();localStorage['CartList_'+goodsshopid]=$(".Cartlist").html();});
									goods.each(function(i,o){
							     		if($(o).attr("goodsId")==goodsId){
							     			$(o).data('count', '0');
							     			$(o).parent().find('.countBuy').html(0);//.hide();
							     		}
					    			});
								}
							}
						...
						if(count>0){
							...
							if(count==0){
								$("#delete_"+goodsId).data('count',"0");	
								$("#delete_"+goodsId).addClass('hide');	
								$(this).parents(".G"+goodsId).slideUp(200,function(){$(this).remove();localStorage['CartList_'+goodsshopid]=$(".Cartlist").html();});
								goods.each(function(i,o){
						     		if($(o).attr("goodsId")==goodsId){
						     			$(o).data('count', '0');
						     			$(o).parent().find('.countBuy').html(0);//.hide();
