...
* @Last Modified by:   Samphay
* @Last Modified time: 2014-11-28 10:51:30
* @Verson: Alpha 1.0
*/
function slider(sliderBox,sliderSwrap,slider){//滑动切换Alpha版
			...
			if(!slider.eq(0).hasClass('on')) slider.eq(0).addClass("on");             //如果第一个slider没有加上on，则会自动帮加上。             
...
function Panel(panelBox,panelHead,panelContent,ifSibling){//面板展开
		...
		function PanelToggle(){
		   ...
		   if(THIS.parent().find(panelContent).is(":hidden")){
			if(ifSibling) panelBox.find(panelContent).fadeOut(100);
			THIS.parent().find(panelContent).fadeIn(100);
			}else{
				THIS.parent().find(panelContent).fadeOut(100);
			}	
		}
}
...
function success(){
			$(".detailBox").html($(".on").html());
			}
$(function(){
	$(".WhiteBoxBottom:before").css({"left":"200px"});
})