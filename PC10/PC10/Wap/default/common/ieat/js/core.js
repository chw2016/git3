...
* @Last Modified by:   星辉
* @Last Modified time: 2015-01-06 14:02:51
* @Verson: Alpha 1.0
*/
function slider(sliderBox,sliderSwrap,slider){//滑动切换Alpha版
            ...
            sliderBox.children().eq(0).children().eq(1).html(sliderLength);
			...
			function OnEnd(e){
				...
				//console.log(EQ);
                sliderBox.children().eq(0).children().eq(0).html(EQ+1);
				if(EQ==0){
					$(".Aleft").hide();
...
function Panel(panelBox,panelHead,panelContent,ifSibling){//面板展开
		...
		function PanelToggle(){
		   ...
		   if(THIS.parent().find(panelContent).is(":hidden")){
			if(ifSibling) {panelBox.find(panelContent).fadeOut(80);panelHead.removeClass('on');}
			THIS.parent().find(panelContent).fadeIn(80);
			THIS.addClass('on');
			}else{
				THIS.parent().find(panelContent).fadeOut(80);
				THIS.removeClass('on');
			}	
		}
}
