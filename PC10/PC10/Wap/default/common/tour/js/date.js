...
(function ($) {      
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        function refreshDate(){
            ...
            dayScroll.refresh();
            resetInitDete();
            yearScroll.scrollTo(0, (nowdate.getFullYear() - opts.beginyear)*40, 100, true);
	   ...
	   function resetIndex(){
			  ...
			  $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               if(datetime){
                ...
                if(Ycallback===undefined){
                     if(docType){that.val(datestr);}else{that.html(datestr+"<div class='tangle'></div>");}
                }else{
                                    Ycallback(datestr);
                }
                $("#datePage").hide(); 
        ...
        function createDAY_UL(){
              $("#daywrapper ul").html("");
              if(indexM == '1' || indexM == '3' || indexM == '5' || indexM == '7' || indexM == '8' || indexM == '10' || indexM == '12'){
                opts.endday = 31;
              };
              if(indexM == '2' && (Number(indexY)+Number(1999)) % 4 == 0) {
                opts.endday = 29;
              };
              if(indexM == '2' && (Number(indexY)+Number(1999)) % 4 != 0){
                opts.endday = 28;
