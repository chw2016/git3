            ...
            if($UserDB->create()){
                ...
                if($user_id){
                    ...
                    if (M("RoleUser")->data($data)->add()){
                        $this->success2('添加成功！','index.php?g=Admin&m=User&a=index');
                    }else{
                        $this->error2('用户添加成功,但角色对应关系添加失败!','index.php?g=Admin&m=User&a=index');
                    }
                }else{
                     $this->error2('添加失败!');
                }
            }else{
                $this->error2($UserDB->getError());
            }
        }else{
        ...
        if(isset($_POST['dosubmit'])) {
            ...
            if($UserDB->create()){
                if($UserDB->save()){
                    ...
                    $this->success2('编辑成功！','index.php?g=Admin&m=User&a=index');
                }else{
                     $this->error2('编辑失败!','index.php?g=Admin&m=User&a=index');
                }
            }else{
                $this->error2($UserDB->getError());
            }
    ...
    public function del(){
        ...
        if($UserDB->delUser('id='.$id)){
            if(M("RoleUser")->where('user_id='.$id)->delete()){
                ...
                $this->success2('删除成功！','index.php?g=Admin&m=User&a=index');
            }else{
                $this->error2('用户成功,但角色对应关系删除失败!');
            }
        }else{
            $this->error2('删除失败!');
        }
