 ...
 * 米业前端
 */
class HyzxAction extends BaseAction{
	...
	public function index(){
	    ...
	    $show       = $Page->show();
	    $list = M('mru_jf')->where(array('token'=>$token,'state'=>1))->field('content',true)->order('sort')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function dh(){
		...
		if($_GET['jfvnum']>$_GET['jfnum']||$_GET['jfvnum']==$_GET['jfnum']){
			...
			echo '<script>alert("该物品库存不足");location.href="'.U('Hyzx/index',array('token'=>$_GET['token'],'openid'=>$_GET['openid'])).'";</script>';die;
		...
		if($_GET['jf']>$_GET['num']){
			...
			echo '<script>alert("积分不够");location.href="'.U('Hyzx/index',array('token'=>$_GET['token'],'openid'=>$_GET['openid'])).'";</script>';die;
		}
		//查当用户地址
		$dz=M('mru_mhyzxsy')->where(array('token'=>$_GET['token'],'openid'=>$_GET['openid']))->getField(dz);
		if(!$dz){
		...
		if(M('mru_jfxx')->where(array('token'=>$_GET['token'],'openid'=>$_GET['openid'],'uid'=>$_GET['uid']))->find()){
			...
			echo '<script>alert("该物品您已经兑换过一次了");location.href="'.U('Hyzx/index',array('token'=>$_GET['token'],'openid'=>$_GET['openid'])).'";</script>';die;
		}else{
			$_GET['dz']=$dz;
			$_GET['add_time']=time();
			M('mru_jfxx')->add($_GET);
			...
			echo '<script>alert("兑换成功");location.href="'.U('Hyzx/index',array('token'=>$_GET['token'],'openid'=>$_GET['openid'])).'";</script>';die;
		}
	}
}
?>
