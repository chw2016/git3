...
class ProductAction extends BaseAction{
	...
	public function __construct(){
		...
		$this->wecha_id	= $_REQUEST['openid'];
		if (!$this->wecha_id){
			$this->wecha_id= $_REQUEST['wecha_id'];
		}
	...
	public function cats(){
		...
		if ($this->isDining || $this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
			$catWhere['dining']=1;
		}else {
			$catWhere['dining']=0;
		}
        ...
        if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
            $this->display('tpl/Wap/theme1/Product_cats.html');
        }else{
            $this->display();
        }
	}
	public function products(){
		...
		//
        if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
            //$products = $this->product_model->where($where)->order($order.' '.$method)->limit('20')->select();
            $products = $this->product_model->where($where)->order($order.' '.$method)->limit('20')->select();
        }else{
        ...
        if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
            $this->display('tpl/Wap/theme1/Product_products.html');
        }else{
            $this->display();
        }
	}
	...
	public function addProductToCart(){//商品id|商品价格|商品数量,
        if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
            $data=$_REQUEST['data'];
            $count=isset($_REQUEST['num'])?intval($_REQUEST['num']):1;
            $id=$data['productId'];
        }else{
		...
		if (key_exists($id,$carts)){
            ...
            if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
			    $carts[$id]=array('count'=>1,'price'=>floatval($data['price']));
            }else{
		...
		$calCartInfo=$this->calCartInfo();
        if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
            $this->ajaxReturn(array('errorCode'=>1,'body'=>'ob','errorMsg'=>'ok'));
            exit;
        }
	...
	public function cart(){
        ...
        if($this->token=="e22e1837a4b8147d6cc6d77c24248ea0"){
            $this->display(TMPL_PATH.'/Wap/theme1/Product_cart.html');
        }else{
            $this->display();
        }
	}
	...
	public function orderCart(){
		...
		if (IS_POST){
			...
			$orderid=substr ($this->wecha_id.time(), -32); 
			$row['orderid']=$orderid;
			$orderid=$row['orderid'];
			//
            ...
            if($buytimestamp){
                ...
                $row['minutes']=$this->_post('minutes');
			...
			if ($cartsCount){
				...
				if (count($diningCart)){
					...
					$row['buytime']=$buytimestamp?$row['month'].'月'.$row['day'].'日'.$row['hour'].'点:'.$row['minutes'].'分':'';
...
// 结束
             ...
             */
            /*
            $notichcontent = $this->wxusers['nickname']."您好".$this->tpl['name']."已经收到您的订单,订单编号:".$dining_rt;
            if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
                $notichcontent .=",我们将在55分钟之内送到您手中,请耐心等候";
            }
            $postdata = array('openid'=>$this->wecha_id,'token'=>$this->token,'content'=>$notichcontent);
            ...
            */
			...
			if ($alipayConfig['open']==0){
				$this->redirect(U('Product/my',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'success'=>1,'dining'=>$_POST['dining'],'tableid'=>$_GET['tableid'])));
			}
		}else {
			//如果是订餐
			...
			if ($cartIsDining){
				...
				$hours=array();
                if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
                    $orderHour=date('H',time())+1;
                    if($orderHour>20||$orderHour<11){
                        $hours=null;
                    }else{
                        for ($i=$orderHour;$i<24;$i++){
                            array_push($hours,$i);
                        }
                    }
                }else{
			...
			//
            $productcartlistmodel = M('Product_cart');
            $lastdata = $productcartlistmodel->field('truename,tel,address')->where(array('wecha_id'=>$this->wecha_id,'token'=>$this->token))->order('time desc')->find();
			$this->assign('metaTitle','购物车结算');
		    $this->assign('lastdata',$lastdata);
//            print_r($diningConfigDetail);exit;
		    $this->assign('diningConfigDetail',$diningConfigDetail);
            if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
                $this->display(TMPL_PATH.'/Wap/theme1/Product_orderCart.html');
            }else{
                $this->assign('tableid',$_GET['tableid']);
                $this->display();
            }
	...
	public function my(){
		...
		//
        if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0'){
            $this->display(TMPL_PATH.'/Wap/theme1/Product_my.html');
        }else{
            $this->display();
        }
	...
	public function deleteOrder(){
		...
		$this->redirect(U('Product/my',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'dining'=>$_GET['dining'],'tableid'=>$_GET['tableid'])));
	}
	...
	public function dining(){
        ...
        if($this->token=='e22e1837a4b8147d6cc6d77c24248ea0s'){
            $this->display(TMPL_PATH.'/Wap/theme1/Product_dining.html');
        }else{
            $this->display();
        }
