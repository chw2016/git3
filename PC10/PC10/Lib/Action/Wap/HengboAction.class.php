...
class HengboAction extends BaseAction
{
    ...
    public function message(){
        ...
        $aOffices['info'] =htmlspecialchars_decode($aOffices['info'],ENT_QUOTES);
        ...
        if(IS_AJAX){
            ...
            if($oCandidateModel->add($aData)){
                FC::mgs($token=$this->_sToken,$openid=$this->openid,
                    $content='您的应聘信息已提交成功，请耐心等待hr回复处理，谢谢！');
            ...
            $this->assign(array(
                ...
                'aXinxi'=>M('Recruitment_info')->where(array('token'=>$this->_sToken,'aid'=>$_GET['aid']))->find()
            ));
        }
    ...
    public function index(){
        ...
        if(!$_GET['aid']){
           $aArea =  $oAreaModel->where(array('token'=>$this->token))->order('id desc')->select();

           $aAreas= $aArea[0];
           // print_r($aAreas);exit;

            if($aAreas){
                header('Location:'.U('Hengbo/index',array('token'=>$this->token,'openid'=>$this->openid,'aid'=>$aAreas['id'])));
            }
        }
        $aAreas = $oAreaModel->where(array('token'=>$this->token,'id'=>$_GET['aid']))->find();
        //print_r($aAreas);exit;
        $this->assign(array(
    ...
    public function myorders(){
        ...
        foreach($iTem as $iKey=>$aValue){
           // $aUserel =$oUsersModel->where(array('token'=>$this->token,'aid'=>$_GET['aid'],'openid'=>$iTem['openid']))->find();
            $aStaff =$oStaffModel->where(array('id'=>$iTem['staff_id']))->find();
            //$iTem[$iKey]['uname'] = $aUserel['name'];
            $iTem[$iKey]['name'] = $aStaff['staff_name'];
            $iTem[$iKey]['phone'] = $aStaff['tel'];
        }
        //print_r($iTem);exit;
        $this->assign(array(
            'aOrders' => $iTem
    ...
    public function photo(){
        ...
        //print_r($aFrist);exit;
        $aList = $oWallModel->join('tp_wx_photo as w ON tp_photo_wall.pid = w.id')->where(array('tp_photo_wall.token'=>$this->token))->order('add_time desc')->limit(6)->select();
        ...
        foreach($aList as $iKey=>$aValur){
           /* $aWall = $oPhotoModel->where(array('id'=>$aValur['pid']))->find();
            ...
            $aList[$iKey]['add_time'] =date('Y/m/d',strtotime($aList[$iKey]['add_time']));
        }
        ...
        if(IS_AJAX){
            $plen = $this->_post('page','intval') * 6;
            $fetchResult = $oWallModel->join('tp_wx_photo as w ON tp_photo_wall.pid = w.id')->where(array('tp_photo_wall.token'=>$this->token))->order('add_time desc')->limit($plen.',6')->select();
             foreach($fetchResult as $iKey=>$aValur){
                 /* $aWall = $oPhotoModel->where(array('id'=>$aValur['pid']))->find();
            $aList[$iKey]['pic'] = $aWall['pic'];*/
                 $fetchResult[$iKey]['add_time'] =date('Y/m/d',strtotime($aValur['add_time']));
             }
