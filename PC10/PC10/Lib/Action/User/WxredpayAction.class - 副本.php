 ...
 *  技术支持：张银飞，张湘南，李铭，周岸毅
 **/
class WxredpaysAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
    ...
    protected function setHeader(){
        return array(
            array(
                'name' => '现金红包活动',
                'url'  => U('Wxredpays/index', array('token' => $this->_sToken))
            ),
    ...
    public function index(){
        ...
        $this->table(
            array(
                ...
                'HeadHover' => U('Wxredpays/index', array('token' => $this->_sToken)),//栏目样式
                'Head_Opt' => array(
                    array(
                        'name'   => '添加现金红包',//2级
                        'url'    => U('Wxredpays/add_content',array('token'=>$_SESSION['token']))
                    )
                ),
                'tips' => array(//3级
                    '你可以在这里管理信息'
                ...
                'Table_Header' => array(//4级
                    'ID', '活动名称', '开始时间', '结束时间', '开启状态','操作'
                ),
                'List_Opt' => array(

                    array(
                        'name' => '查看用户',
                        'url'  => U('Wxredpays/reduser',array('token'=>$_SESSION['token']))
                    ),

                    array(
                        'name' => '编辑',
                        'url'  => U('Wxredpays/save_content',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('Wxredpays/delete_content',array('token'=>$_SESSION['token']))
                    ),
                ),
                /*         		//搜索
                                'search'=>array(
            ...
            M('Wxredpay')->field('id,act_name,strattime,endtime,is_open')->where(array('token'=>$_SESSION['token']))->order("sort")
        // array($this,'abc')
        );
    }
    ...
    public function add_save($aaa){
        //特殊点选框,复选框,下拉列表
        /* 		$list=M('Wxredpays2')->select();//被添加内容的表
                foreach ($list as $k=>$v){
        ...
        $this->$aaa('Wxredpay',array(
             ...
             * */
            array('title'=>"活动名称",'type'=>"longinput",'name'=>"act_name",'value'=>'act_name','msg'=>'请填写活动的名称'),
            array('title'=>"供方名称",'type'=>"longinput",'name'=>"nick_name",'value'=>'nick_name','msg'=>'请填写如：天虹百货','bast'=>'提供方名称'),
            array('title'=>"商户名称",'type'=>"longinput",'name'=>"send_name",'value'=>'send_name','msg'=>'请填写如：天虹百货','bast'=>'红包发送者名称'),
            array('title'=>"红包总金额",'type'=>"number",'name'=>"total_amount",'value'=>'total_amount','msg'=>'请填写红包的总金额'),
            array('title'=>"红包总个数",'type'=>"number",'name'=>"total_num",'value'=>'total_num','msg'=>'请填写红包的总的个数'),
            array('title'=>"最小红包金额",'type'=>"number",'name'=>"min_value",'value'=>'min_value','msg'=>'请填最小红包金额','bast'=>'最少不低于1元'),
            array('title'=>"最大红包金额",'type'=>"number",'name'=>"max_value",'value'=>'max_value','msg'=>'请填最大红包金额','bast'=>'最多不高于200元'),
            array('title'=>"开始时间",'type'=>"time",'name'=>"strattime",'msg'=>'请填写开始时间','value'=>'strattime'),
            array('title'=>"结束时间",'type'=>"time",'name'=>"endtime",'msg'=>'请填写结束时间','value'=>'endtime'),
            array('title'=>"每天领取的最多个数",'type'=>"number",'name'=>"daynum",'value'=>'daynum','msg'=>'请填每天领取的最多个数'),
            array('title'=>"每人领取的最多个数",'type'=>"number",'name'=>"max_value",'value'=>'max_value','msg'=>'请填每人领取的最多个数'),
            ...
            //array('title'=>"input框",'type'=>"input",'name'=>"name",'value'=>'pinzhong','msg'=>'请填写input框','bast'=>'备注说明'),
        ...
        ),U('Wxredpays/index',array('token'=>$_SESSION['token']))/*,array($this,'bbc')*/);
...
//其它信息开头
        ...
        $this->table(
            array(
                ...
                'Head_Opt' => array(
                    array(
                        'name'   => '返回',//2级
                        'url'    => U('Wxredpays/index',array('uid'=>$_GET['id']))
                    )
                ),
                'tips' => array(//3级
                    '你可以在这里管理信息'
                ...
                'List_Opt' => array(
                    ...
                    array(
                        'name' => '查看',
                        'url'  => U('Wxredpays/save_xx',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('Wxredpays/delete_xx',array('token'=>$_SESSION['token']))
                    ),
                ),

            ),
            M('Wxredpay')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id']))->count(),
            M('Wxredpay')->field('id')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id'])),
            array($this,'xxabc')
        );
    }
    ...
    public function add_save_xx($aaa){
        ...
        ),U('Wxredpays/xx',array('token'=>$_SESSION['token'],'id'=>$_GET['uid'])),array($this,'bbc'),1);
