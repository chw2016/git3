 ...
 *  技术支持：张银飞 ,李铭,张湘南
 **/
class TestanyiAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
    ...
    protected function setHeader(){
        return array(
            array(
                'name' => '标题',
                'url'  => U('Testanyi/index', array('token' => $this->_sToken))
            ),
    ...
    public function index(){
        ...
        $this->table(
            array(
                ...
                'HeadHover' => U('Testanyi/index', array('token' => $this->_sToken)),//栏目样式
                'Head_Opt' => array(
                    array(
                        'name'   => '添加',//2级
                        'url'    => U('Testanyi/add_content',array('token'=>$_SESSION['token']))
                    )
                ),
                'tips' => array(//3级
                    '你可以在这里管理信息'
                ...
                'List_Opt' => array(
                    ...
                    /*               			array(
                                                    'name' => '其它信息',
                                                      'url'  => U('Testanyi/xx',array('token'=>$_SESSION['token']))
                                            ), */

                    array(
                        'name' => '编辑',
                        'url'  => U('Testanyi/save_content',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('Testanyi/delete_content',array('token'=>$_SESSION['token']))
                    ),
                ),
                /*         		//搜索
                                'search'=>array(
            ...
            M('Anyitest')->where(array('token'=>$_SESSION['token']))->count(),
            M('Anyitest')->field('id,name')->where(array('token'=>$_SESSION['token']))->order("id desc")
        // array($this,'abc')
        );
    }
    ...
    public function add_save($aaa){
        ...
        ),U('Testanyi/index',array('token'=>$_SESSION['token'])),array($this,'bbc'));
    ...
    public function delete_content(){
        //M('Anyitest')->where(array('uid'=>$_GET['id']))->delete();//删除其它信息
        $this->del('Anyitest');
    }
        ...
        $this->table(
            array(
                ...
                'Head_Opt' => array(
                    array(
                        'name'   => '返回',//2级
                        'url'    => U('Testanyi/index',array('uid'=>$_GET['id']))
                    )
                ),
                'tips' => array(//3级
                    '你可以在这里管理信息'
                ...
                'List_Opt' => array(
                    ...
                    array(
                        'name' => '查看',
                        'url'  => U('Testanyi/save_xx',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('Testanyi/delete_xx',array('token'=>$_SESSION['token']))
                    ),
                ),

            ),
            M('Anyitest')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id']))->count(),
            M('Anyitest')->field('id')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id'])),
            array($this,'xxabc')
        );
    }
    ...
    public function add_save_xx($aaa){
        ...
        ),U('Testanyi/xx',array('token'=>$_SESSION['token'],'id'=>$_GET['uid'])),array($this,'bbc'),1);
