 ...
 *  银波米业
 **/
class ZhaopinAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
    ...
    {
    	parent::_initialize();
        $this->pz	   = D('mru_zhaopin');//招聘
        $this->dp	   = D('mru_zhaopin');//加盟
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zhaopin/index', array('token' => $this->_sToken)),
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加招聘信息',
                    	'url'    => U('Zhaopin/add_pinzhong')
                	),
            			array(
            					'name'   => '我要加盟',
            					'url'    => U('Zhaopin/index2')
            			)
            	),
                'tips' => array(
                    '你可以在这里管招聘信息'
            	...
            	'List_Opt' => array(
                	...
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zhaopin/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zhaopin/PinzhongDel')
	                ),
            	)
        	),
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,zw,bm,sex,age,xl,jy,sort')->order("sort")->where($aWhere),
				array($this,'abc')
...
/*                     array(
                ...
                'Table_Header' => array(
                    'ID', '申请人','学历', '电话','从事职业','性别','年龄', '店面团队人数','现有投资规模','拟投资规模','加盟时间','查看'
                ),
                'List_Opt' => array(
                    array(
                        'name' => '查看详情',
            ...
            M('mru_wyjm')->field('id,name,xl,dh,zy,age,sex,num,xtz,ytz,add_time')->order("add_time desc")->where($aWhere),
...
/*                     array(
                ...
                'List_Opt' => array(
                    array(
                        'name' => '查看详情',
                        'url'  => U('Zhaopin/jlk',array('uid'=>$_GET['id']))
                    ),
                     array(
                        'name' => '删除',
                        'url'  => U('Zhaopin/jlk2')
	...
	public function dianpuAdd(){
		if(IS_POST){
            ...
            $this->assign(array(
                ...
                'FormUrl' => U('Zhaopin/dianpuAdd',array('token'=>$_SESSION['token']))
            ));
	...
	public function dianpuEdit(){
		...
		if(IS_POST){
			...
			$this->assign(array(
                'info' => $Item,
                'id' => $iID,
                'FormUrl' => U('Zhaopin/dianpuEdit',array('token'=>$_SESSION['token'])),
                'ExtraBtn' => array(
                    array(
                        'url'  => U('Zhaopin/dianpu', array('token' => $this->_sToken)),
                        'name' => '返回'
                )),
            ));
            $this->UDisplay('dianpu_add');
	...
	public function order(){
        ...
        $this->assign(array(
            //'id' => 'id',//如果主键不是id，则需要设置
            'HeadHover' => U('Zhaopin/order', array('token' => $this->_sToken)),
            'Head_Opt' => array(),
            'Table_Header' => array(
                'ID', '品种', '手机号', '店铺名',
        		'地址', '描述','状态', '下单时间',
            ...
            'List_Opt' => array(
                array(
                    'name' => '查看详细',
                    'url'  => U('Zhaopin/OrderShow', array('token' => $this->_sToken))
                ),
                array(
                    'name' => '完成',
                    'url'  => U('Zhaopin/orderHandle', array(
                    	'token' => $this->_sToken,
    ...
    public function PinzhongEdit(){
        ...
        ),U('Zhaopin/index', array('token' => $this->token)));
    ...
    public function PinzhongDel(){
        ...
        $this->del('mru_zhaopin');
    }
    
    public function ck(){
     	$id=$_GET['id'];
     	$list=M('mru_wyjm')->where(array('id'=>$id))->find();
    ...
    public function ck2(){
    	$id=$_GET['id'];
    	$list=M('mru_wyjm')->where(array('id'=>$id))->delete();
    	if($list){
    		$this->error2("删除成功");
    	}else{
    		$this->error2("删除失败");
...
/*     							array(
    ...
    public function savejm(){
    	...
    	),U('Zhaopin/addjm', array('token' => $this->token)));
    
    }
    
    //排序  只 能主健是id,子段是sort 才能排序  有意见自己去开发！
    public function sortajax(){
    	$this->sortajaxTable('mru_zhaopin');
    }
