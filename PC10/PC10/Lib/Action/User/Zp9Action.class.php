 ...
 *  银波米业1
 **/
class Zp9Action extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/*     			array(
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zp9/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加 内容',//2级
                    	'url'    => U('Zp9/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理内容信息'
            	...
            	'Table_Header' => array(//4级
                	'ID', '文字', '屏幕上方图片', '屏幕中间图片','背景图','操作'
            	),
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zp9/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zp9/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,name,pic,pic2,pic3')->where($aWhere),
            array($this,'abc')
         );
	}
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_Zp9 pz on pz.id = o.pinzhong_id')
...
/* 		$list=M('Zp92')->select();//被添加内容的表
		...
		$this->$aaa('Zp9',array(
				array('title'=>"文字",'type'=>"input",'name'=>"name",'value'=>'name'),
				array('type'=>'img','many'=>array(
						array('title'=>"屏幕上方图片",'type'=>"img",'name'=>"pic",'value'=>'pic','width'=>280,'height'=>73),
						array('title'=>"屏幕中间图片",'type'=>"img",'name'=>"pic2",'value'=>'pic2','width'=>142,'height'=>142),
						array('title'=>"背景图",'type'=>"img",'name'=>"pic3",'value'=>'pic3','width'=>142,'height'=>142),
				)),
    ...
    public function add_pinzhong(){
    	...
    	$b=M('Zp9')->where(array('token'=>$token))->find();
    	if($b){
