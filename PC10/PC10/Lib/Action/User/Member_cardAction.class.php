...
class Member_cardAction extends UserAction{
	...
	public function news(){
		...
		if(IS_POST){
			...
			if($data==false){				
				$this->all_insert('Member_card_info','/news');
			}else{
				$_POST['id']=$data['id'];
				$this->all_save('Member_card_info','/news');
			}
		}else{
	...
	public function index(){
		...
		$data=M('Member_card_set')->where(array('token'=>session('token')))->find();
		if(IS_POST){
  ...
  'create_time' => '0',
	...
	public function privilege_edit(){
		if(IS_POST){
			...
			$data=M('Member_card_vip')->where(array('token'=>session('token'),'id'=>$_REQUEST['id']))->find();
			if($data!=false){
	...
	public function privilege_del(){
		...
		$data=M('Member_card_create')->where(array('token'=>session('token'),'wecha_id'=>$_REQUEST['id']))->delete();
        $data1=M('Member_card_sign')->where(array('token'=>session('token'),'wecha_id'=>$_REQUEST['id']))->delete();
        $data2=M('Userinfo')->where(array('token'=>session('token'),'wecha_id'=>$_REQUEST['id']))->delete();
        if($data==false){
            $this->error('服务器繁忙请稍后再试');
        }else{
            $this->success('操作成功',U('Member_card/create',array('token'=>session('token'))));
	...
	public function coupon_edit(){
		if(IS_POST){
			...
			$data=M('Member_card_coupon')->where(array('token'=>session('token'),'id'=>$_REQUEST['id']))->find();
			if($data!=false){
				$this->assign('vip',$data);
				$this->display();
			}else{
	...
	public function coupon_del(){
			$data=M('Member_card_coupon')->where(array('token'=>session('token'),'id'=>$_REQUEST['id']))->delete();
			if($data==false){
				$this->error('服务器繁忙请稍后再试');
			}else{
	...
	public function integral_edit(){
		if(IS_POST){			
			...
			$data=M('Member_card_integral')->where(array('token'=>session('token'),'id'=>$_REQUEST['id']))->find();
			if($data!=false){
				$this->assign('vip',$data);
				$this->display();
			}else{
	...
	public function integral_del(){
			$data=M('Member_card_integral')->where(array('token'=>session('token'),'id'=>$_REQUEST['id']))->delete();
			if($data==false){
				$this->error('服务器繁忙请稍后再试');
			}else{
