 ...
 *  银波米业1
 **/
class PpjsAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Ppjs/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '品牌介绍',//2级
                    	'url'    => U('Ppjs/index')
            	...
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Ppjs/PinzhongEdit')
                	),
/* 	                array(
	                    'name' => '删除',
	                    'url'  => U('Ppjs/PinzhongDel')
	                ), */
            	)
        	), 
        	$this->pz->where(array('token'=>session(token)))->count(),
        	$this->pz->field('id,name')->where(array('token'=>session(token))),
            array($this,'abc')
         );
	}
	...
	public function abc($data){
		foreach($data as $k=>$v){
		//$data[$k]['content']=htmlspecialchars_decode(mb_substr($v['content'],0,32,'utf-8'));
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_mru_ppjs pz on pz.id = o.pinzhong_id')
	...
	public function addEdit($aaa){
		//特殊点选框,复选框,下拉列表
/* 		$list=M('Ppjs2')->select();//被添加内容的表
		foreach ($list as $k=>$v){
			$list[$k]['content']=$v['name'];//把内容子段改成content
			$list[$k]['value']=$v['id'];//把id子段改成value
			unset($list[$k]['name']);//删除原来的内容子段
    	} */
		$this->$aaa('mru_ppjs',array(
				
				
				array('title'=>"简介",'type'=>"textarea",'name'=>"content",'value'=>'content'),
				
		),U('Ppjs/index'),array($this,'bbc'));
	}
...
//其它信息开始
    	...
    	$this->table(
    			array(
    					...
    					'List_Opt' => array(
    							array(
    									'name' => '编辑',
    									'url'  => U('Ppjs/save_content')
    							),
    							 	                array(
    							 'name' => '删除',
    									'url'  => U('Ppjs/delete_content')
    							),
    					)
    			),
    			M('mru_fc')->where(array('token'=>session(token)))->count(),
    			M('mru_fc')->field('id,name,pic,sort')->order("sort")->where(array('token'=>session(token))),
    			array($this,'xxabc')
    	);
    }
    ...
    public function add_save_xx($aaa){
    	...
    	$this->$aaa('mru_fc',array(
    			//array('title'=>"较长input框",'type'=>"longinput",'name'=>"cname",'value'=>'cname','msg'=>'请填写较长input框1','tishi'=>'提示:内容'),
    			array('title'=>"标题",'type'=>"input",'name'=>"name",'value'=>'name','msg'=>'请填写input框1'),
    			
    			
    			array('type'=>'img','many'=>array(
    					array('title'=>"图片",'type'=>"img",'name'=>"pic",'value'=>'pic','width'=>320,'height'=>200),
    			)),
    			
    			
    			//array('title'=>"密码",'type'=>"password",'name'=>"pwd",'value'=>'pwd','msg'=>'请填写标题咯'),
    			//array('title'=>"隐藏",'type'=>"hidden",'name'=>"id",'msg'=>'请填写标题咯1','value'=>'id'),
    			//array('title'=>"开始时间",'type'=>"time",'name'=>"k_time",'msg'=>'请填写开始时间1','value'=>'k_time'),
    			//array('title'=>"结束时间",'type'=>"time",'name'=>"j_time",'msg'=>'请填写结束时间1','value'=>'j_time'),
    			//array('title'=>"数量",'type'=>"number",'name'=>"num",'msg'=>'','value'=>'num','msg'=>'请填写数量1',),
    
    			//array('type'=>'radio','title'=>"特殊点选框",'name'=>"sex",'value'=>'sex','msg'=>'请填写标题咯1','many'=>$list),
    			//array('type'=>'select','title'=>"下拉列表",'name'=>"sex2",'value'=>'sex2','msg'=>'请选择下拉列表1','many'=>array(
    	...
    	),U('Ppjs/fc',array('token'=>$_SESSION['token'])),array($this,'bbc'));
