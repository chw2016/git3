...
class GrouponAction extends UserAction{
	...
	public function _initialize() {
		...
		/*if(!strpos($token_open['queryname'],'etuan')){
            $this->error2('您未启该模块的使用权,请到功能模块中添加',U('Function/index',array('token'=>session('token'),'id'=>session('wxid'))));
		}*/
	...
	public function add(){ 
		...
		if(IS_POST){
			...
			if($this->product_model->add($_POST)){
				$this->success('添加成功',U('Groupon/products',array('token'=>$this->token)));
			}else{
				$this->error2('操作失败');
			}
			...
			if (!$cats){
				 $this->error2("请先添加分类",U('Product/catAdd',array('token'=>$this->token)));
				 exit();
			}
	...
	public function del(){
        ...
        if(IS_POST){
            ...
            $back=$this->product_model->where($where)->delete();
            if($back==true){
            	$keyword_model=M('Keyword');
            	$keyword_model->where(array('token'=>$this->token,'pid'=>$id,'module'=>'Product'))->delete();
                $this->success2('操作成功',U('Groupon/products',array('token'=>$this->token)));
            }else{
                 $this->error2('服务器繁忙,请稍后再试',U('Groupon/products',array('token'=>$this->token)));
            }
        }        
	}
	public function index(){
		if (IS_POST){
			...
			$this->success2('操作成功',U('Groupon/index',array('token'=>session('token'),'dining'=>$this->isDining)));
		}else{
			$where=array('token'=>$this->token,'groupon'=>1);
			if(IS_POST){
				$key = $this->_post('searchkey');
