 ...
 *  银波米业
 **/
class FuwuAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Fuwu/index', array('token' => $this->_sToken)),
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加服务',
                    	'url'    => U('Fuwu/add_pinzhong')
                	)
            	),
                'tips' => array(
                    '你可以在这里管理添加服务信息'
            	...
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Fuwu/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Fuwu/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,add_time,sort')->order("sort")->where($aWhere),
				array($this,'abc')
         );
	}
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_mru_fuwu pz on pz.id = o.pinzhong_id')
    ...
    public function add_pinzhong(){
        ...
        $this->add('mru_fuwu',array(
  
        		array('type'=>'select','title'=>"类型",'name'=>"name",'value'=>'name','msg'=>'请填写标题咯','many'=>array(
        				array('value'=>'', 'content'=>'请选择类型'),
        				array('value'=>'1', 'content'=>'减肥'),
        				array('value'=>'2','content'=>'美容'),
        				array('value'=>'3','content'=>'美体'),
        				array('value'=>'4','content'=>'亚健康'),
        		)),
  
		   array('type'=>'img','many'=>array(
                array('title'=>"列表图片",'type'=>"img",'name'=>"pic",'value'=>'pic','width'=>'320','height'=>'101'),
		/*    		array('title'=>"内容图片",'type'=>"img",'name'=>"contentPic",'value'=>'contentPic'), */
            )),

            array('title'=>"图文详细",'type'=>"textarea",'name'=>"content",'value'=>'content'),

        ),U('Fuwu/index'),array($this,'bbc'));
    ...
    public function PinzhongEdit(){
        ...
        ),U('Fuwu/index', array('token' => $this->token)));
    ...
    public function sortajax(){
    	$this->sortajaxTable('mru_fuwu');
    }
    
    //全选操作
    public function qx(){
    	$id=implode(',', $_REQUEST['list']);
    	if(!$id) script("全选后此操作才有效");
