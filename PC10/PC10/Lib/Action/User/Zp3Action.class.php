 ...
 *  银波米业1
 **/
class Zp3Action extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/*     			array(
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zp3/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加图片',//2级
                    	'url'    => U('Zp3/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理品种信息'
            	...
            	'Table_Header' => array(//4级
                	'ID', '背景图片', '屏幕上方图片', '中间文字图片1', '中间文字图片2', '中间文字图片3', '中间文字图片4','操作'
            	),
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zp3/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zp3/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,pic2,pic3,pic4,pic5,pic6')->where($aWhere),
            array($this,'abc')
         );
	}
	...
	public function abc($data){
		foreach($data as $k=>$v){
			...
			$data[$k]['pic3']="<img style='width:50px' height:50px; src='{$v['pic3']}' />";
			$data[$k]['pic4']="<img style='width:50px' height:50px; src='{$v['pic4']}' />";
			$data[$k]['pic5']="<img style='width:50px' height:50px; src='{$v['pic5']}' />";
			$data[$k]['pic6']="<img style='width:50px' height:50px; src='{$v['pic6']}' />";
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_Zp3 pz on pz.id = o.pinzhong_id')
...
/* 		$list=M('Zp32')->select();//被添加内容的表
		...
		$this->$aaa('Zp3',array(
				...
				array('type'=>'img','many'=>array(
						...
						array('title'=>"屏幕上方图片",'type'=>"img",'name'=>"pic2",'value'=>'pic2','width'=>327,'height'=>209),
						array('title'=>"中间文字图片1",'type'=>"img",'name'=>"pic3",'value'=>'pic3','width'=>50,'height'=>134),
						array('title'=>"中间文字图片2",'type'=>"img",'name'=>"pic4",'value'=>'pic4','width'=>52,'height'=>140),
						array('title'=>"中间文字图片3",'type'=>"img",'name'=>"pic5",'value'=>'pic5','width'=>51,'height'=>137),
						array('title'=>"中间文字图片4",'type'=>"img",'name'=>"pic6",'value'=>'pic6','width'=>51,'height'=>137),
    ...
    public function add_pinzhong(){
    	...
    	$b=M('Zp3')->where(array('token'=>$token))->find();
    	if($b){
    		echo "<script> alert('您已经添加过了');history.back(); </script> ";die;
    	}
	    $this->addEdit(add);
