 ...
 *  银波米业1
 **/
class MruHbAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('MruHb/index', array('token' =>$_SESSION['token'])),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加优惠卷',//2级
                    	'url'    => U('MruHb/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
            	...
            	'List_Opt' => array(
            			...
            			array(
            					'name' => '此优惠卷拥有者',
            					'url'  => U('MruHb/xx',array('token'=>$_SESSION['token']))
            			),
                	...
                	array(
                    	'name' => '编辑',
                    	'url'  => U('MruHb/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('MruHb/PinzhongDel')
	                ),
...
/* 			switch ($v['state']){
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_mru_hb pz on pz.id = o.pinzhong_id')
...
/* 		$list=M('MruHb2')->select();//被添加内容的表
		...
		),U('MruHb/index',array('aid'=>$aid)),array($this,'bbc'));
	}
   ...
   /**
    ...
    public function xx(){
    	...
    	if(IS_POST){
    		...
    		if(M('mru_hb')->where(array('id'=>$_GET['id']))->getField('state')) script("已使用");
    		$b=M('mru_hb')->where(array('id'=>$_GET['id']))->save(array('state'=>1,'add_time'=>time(),'aid'=>$_SESSION['aid']));
    		
    		$b=M('mru_hb')->where(array('yzm'=>$_POST['eq_yzm']))->getField('state');
    		if($b==1){
    			echo "<script>alert('此红包已被使用');</script>";
    		}
    		if($b!=1){
    			$bb=M('mru_hb')->where(array('yzm'=>$_POST['eq_yzm']))->save(array('state'=>1,'add_time'=>time(),'aid'=>$_SESSION['aid']));
    	...
    	$this->table(
    			array(
    					'abc'=>123,
    					//'id' => 'id',//如果主键不是id，则需要设置
    					'HeadHover' => U('MruHb/xx', array('token' =>$_SESSION['token'])),//栏目样式
    					'Head_Opt' => array(
    /* 							array(
    									'name'   => '返回',//2级
    									'url'    => U('MruHb/index',array('uid'=>$_GET['id']))
    							) */
    					),
    					'tips' => array(//3级
    					...
    					'List_Opt' => array(
    						...
    						/* 	array(
    									'name' => '使用',
    									'url'  => U('MruHb/Sy',array('uid'=>$_GET['id']))
    							),
    							 */
    							
    							array(
    									'name' => '删除',
    									'url'  => U('MruHb/xDel',array('uid'=>$_GET['id']))
    							),
    					),
    			...
    			M('mru_hb')->field('id,name,yzm,price,state,add_time,openid')->where($aWhere)->order("add_time desc"),
    			array($this,'xxabc')
    ...
    public function xxDel(){
    	$j_time=M('mru_hb')->where(array('id'=>$_GET['uid']))->getField(j_time);
    ...
    public function Fj(){
    	...
    	if(IS_POST){
    		...
    		),U('MruHb/index',array('aid'=>$aid)),array($this,'bbc'),'',array('token'=>$_SESSION['token']));
