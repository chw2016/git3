 ...
 *  技术支持：张银飞 ,李铭,张湘南
 **/
class YxXbAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
    ...
    public function index(){
        ...
        $this->table(
            array(
                ...
                'HeadHover' => U('YxXb/index', array('token' => $this->_sToken)),//栏目样式
                'Head_Opt' => array(
                    array(
                        'name'   => '添加新品抢先',//2级
                        'url'    => U('YxXb/add_content',array('token'=>$_SESSION['token']))
                    ),
                ),
                'tips' => array(//3级
                    '你可以在这里管理信息'
                ...
                'List_Opt' => array(
                    ...
                    /*               			array(
                                                    'name' => '其它信息',
                                                      'url'  => U('YxXb/xx',array('token'=>$_SESSION['token']))
                                            ), */

                    array(
                        'name' => '编辑',
                        'url'  => U('YxXb/save_content',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('YxXb/delete_content',array('token'=>$_SESSION['token']))
                    ),
                ),
             
            ),
            M('Yanxiang_xb')->count(),
            M('Yanxiang_xb')->field('id,title,pic,times')->order("times desc"),
       array($this,'abc')
        );
    }
...
/*        	array('title'=>"链接",'type'=>"input",'name'=>"url",'value'=>'url','tishi'=>'<a style="color:red;">注意:如果镇了链接前台跳的是链接地址,前面加http://</a>'),*/
        ...
        ),U('YxXb/index',array('token'=>$_SESSION['token'])),array($this,'bbc'));
    ...
    public function delete_content(){
        //M('Yanxiang_xb')->where(array('uid'=>$_GET['id']))->delete();//删除其它信息
        $this->del('Yanxiang_xb');
    }
    //排序  只 能主健是id,子段是sort,才能排序 .有意见开发！
    public function sortajax(){
        $this->sortajaxTable('Yanxiang_xb');
    }
    //是否显示  只 能主健是id,子段是is_show 才能是否显示  有意见自己去开发！
    public function is_showAjax(){
    	$this->is_showAjaxTable('Yanxiang_xb');
    }
