 ...
 *  银波米业1
 **/
class Zp2Action extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/*     			array(
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zp2/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加图片',//2级
                    	'url'    => U('Zp2/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理图片信息'
            	...
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zp2/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zp2/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,pic2')->where($aWhere),
            array($this,'abc')
         );
	}
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_Zp2 pz on pz.id = o.pinzhong_id')
...
/* 		$list=M('Zp22')->select();//被添加内容的表
		...
		$this->$aaa('Zp2',array(
				...
				array('type'=>'img','many'=>array(
						...
						array('title'=>"屏幕下方图片",'type'=>"img",'name'=>"pic2",'value'=>'pic2','width'=>310,'height'=>118),
				)),
    ...
    public function add_pinzhong(){
    	...
    	$b=M('Zp2')->where(array('token'=>$token))->find();
    	if($b){
