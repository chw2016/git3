 ...
 *  万普分期系统
 **/
class SgAction extends Table1Action {
    ...
    protected function setHeader(){
    	return array(
            ...
            array(
                'name' => '申请分销商',
                'url'  => U('people', array('token' => $this->_sToken))
            ),
            array(
                'name' => '楼盘列表',
	...
	public function index(){
            ...
            $this->table(
                array(
                  ...
                  //  'id' => 'name',//如果主键不是id，则需要设置
                    ...
                    'Head_Opt' => array(
                        array(
                            'name' => '添加分销商类型',
                            'url' => U('add_index',array('token'=>$this->token))
                        ),
                ...
                M('Media_fl')->where($aWhere)->count(),
                M('Media_fl')->field('id,name')->where($aWhere)
    ...
    public function add_index(){
        $this->add('Media_fl',array(
            array('title'=>"类型名称",'type'=>"input",'name'=>"name",'msg'=>'名称不能为空'),

        ),U('index',array('token'=>$this->token)));
    }
    public function edit_index(){
        $this->Edit('Media_fl',array(
            array('title'=>"类型名称",'type'=>"input",'name'=>"name",'msg'=>'名称不能为空'),
    ...
    public function people(){
        ...
        $this->table(
            array(
                //'abc' => 123,
                //  'id' => 'name',//如果主键不是id，则需要设置
                'HeadHover' => U('people', array('token' => $this->_sToken)),
                'Head_Opt' => array(
                    /*  array(
                          'name' => '添加非信贷产品',
