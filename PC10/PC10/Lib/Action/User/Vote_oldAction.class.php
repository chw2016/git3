...
class VoteAction extends UserAction {
	public function _initialize() {
		...
		/*if(!strpos($token_open['queryname'],'Vote')){
            $this->error2('您还开启该模块的使用权,请到功能模块中添加',U('Function/index',array('token'=>session('token'),'id'=>session('wxid'))));
		}*/
	}
	...
	public function add() {
		...
		if (IS_POST) {
			...
			$isset_keyword = $data->where ( array (
			...
			if ($data->create () != false) {
				if ($id = $data->add ()) {
					...
					M ( 'keyword' )->add ( $data1 );
					$this->success2 ( '添加成功', U ( 'Vote/index', array (
							'token' => session ( 'token' ) 
					) ) );
				} else {
	...
	public function del() {
		...
		if ($result) {
			...
			$this->success2 ( '删除成功', U ( 'Vote/index', array (
					'token' => session ( 'token' ) 
			) ) );
		} else {
			$this->error2 ( '非法操作！' );
		...
		if (IS_POST) {
			...
			if ($data->create ()) {
				if ($data->where ( $where )->save ( $_POST )) {
					...
					$this->success2 ( '修改成功', U ( 'Vote/index', array (
							'token' => session ( 'token' ) 
					) ) );
					exit ();
				} else {
