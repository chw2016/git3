 ...
 *  基本表单样式 1
 **/
abstract class CmsAction extends UserAction{
    /**
     *  定义项目模板BaseDir
     **/
     ...
     * $model代表、表名，
     */                
    public function add($model,$input,$url,$fun='',$baoc='',$conditions='',$aheaddata=array(),$gl='',$lmgl=''){
        if(IS_POST){
        	...
        	if($lmgl){
        		...
        		foreach ($input as $ke=>$v){
        			if(!in_array($v['name2'], $lmgl)) unset($input[$ke]);
        			 foreach ($v['many'] as $kee=>$vv){
        				if(!in_array($vv['name2'], $lmgl)){
        					if($vv['name2']=='namepic'||$vv['name2']=='cpic'||$vv['name2']=='pic3'||$vv['name2']=='pic4'||$vv['name2']=='pic5'){
        						unset($input[$ke]['many'][$kee]);
        					}
        				} 
        			}
        		}
     ...
     * $lmgl 选择栏目功能
     */
    public function Edit($model1,$input,$url,$fun='',$baoc='',$conditions='',$aheaddata=array(),$gl='',$lmgl=''){
        if(IS_POST){
	       	$_POST=$_REQUEST;//复选框取值
        	foreach ($_POST as $key=>$v){
        		if(is_array($v)){//如果是个数组转化成字符串，键值分开
        	...
        	if($gl){
        		$gl=explode(',', $gl);
        		foreach ($input as $ke=>$v){
        			if($v['name']!='content') if(!in_array($v['name'], $gl)) unset($input[$ke]);
        		}
        	}
        	//选择栏目功能
        	if($lmgl){
...
/*     //是否显示 只 能主健是id,子段是is_show 才能是否显示  有意见自己去开发！
    public function is_showAjaxTable($Model	= null){
    ...
    public function stateAjaxTable($Model	= null){
