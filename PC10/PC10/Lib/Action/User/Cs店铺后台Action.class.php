...
class CsAction extends TableAction {
    ...
    protected function setHeader(){
    	return array(
                array(
                    'name' => '标题',
                    'url'  => U('Cs/index', array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))
                ),
...
/*     	//搜索
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Cs/index', array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid'])),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加',//2级
                    	'url'    => U('Cs/add_content',array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理信息'
...
/*               			array(
            					'name' => '其它信息',
              					'url'  => U('Cs/xx',array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))
            			), */
            			
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Cs/save_content',array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Cs/delete_content',array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))
	                ),
            	),
/*         		//搜索
	        	'search'=>array(
        	...
        	M('Miye_pinzhong')->where(array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))->count(),
        	M('Miye_pinzhong')->field('id')->where(array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))->order("id desc")
           // array($this,'abc')
         );
	}
...
/* 				array('title'=>"密码",'type'=>"password",'name'=>"pwd",'value'=>'pwd'),
				...
				array('type'=>'img','many'=>array(
						...
						array('title'=>"图片2",'type'=>"img",'name'=>"pic2",'value'=>'pic','width'=>50,'height'=>50),
						array('title'=>"图片2",'type'=>"img",'name'=>"pic3",'value'=>'pic','width'=>50,'height'=>50)
				)),
		...
		),U('Cs/index',array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid'])),array($this,'bbc'));
    	...
    	$this->table(
    			array(
    					...
    					'List_Opt' => array(
    							...
    							array(
    									'name' => '查看',
    									'url'  => U('Cs/save_xx',array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))
    							),
    							array(
    									'name' => '删除',
    									'url'  => U('Cs/delete_xx',array('token'=>$_SESSION['token'],'aid'=>$_SESSION['aid']))
    							),
    					),

    			),
    			M('Miye_pinzhong')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id'],'aid'=>$_SESSION['aid']))->count(),
    			M('Miye_pinzhong')->field('id')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id'],'aid'=>$_SESSION['aid'])),
    			array($this,'xxabc')
    	);
    }
