 ...
 *  银波米业1
 **/
class Zp6Action extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/*     			array(
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zp6/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加图片',//2级
                    	'url'    => U('Zp6/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理品种信息'
            	...
            	'Table_Header' => array(//4级
                	'ID', '背景图片', '图片1', '图片2', '图片3', '图片4', '操作'
            	),
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zp6/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zp6/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,pic2,pic3,pic4,pic5')->where($aWhere),
            array($this,'abc')
         );
	}
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_Zp6 pz on pz.id = o.pinzhong_id')
...
/* 		$list=M('Zp62')->select();//被添加内容的表
		...
		$this->$aaa('Zp6',array(
				...
				array('type'=>'img','many'=>array(
						...
						array('title'=>"图片1",'type'=>"img",'name'=>"pic2",'value'=>'pic2','width'=>327,'height'=>201),
						array('title'=>"图片2",'type'=>"img",'name'=>"pic3",'value'=>'pic3','width'=>132,'height'=>201),
						array('title'=>"图片3",'type'=>"img",'name'=>"pic4",'value'=>'pic4','width'=>132,'height'=>201),
						array('title'=>"图片4",'type'=>"img",'name'=>"pic5",'value'=>'pic5','width'=>132,'height'=>201),
    ...
    public function add_pinzhong(){
    	...
    	$b=M('Zp6')->where(array('token'=>$token))->find();
    	if($b){
