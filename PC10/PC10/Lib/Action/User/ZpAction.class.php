 ...
 *  银波米业1
 **/
class ZpAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/*     			array(
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zp/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加图片',//2级
                    	'url'    => U('Zp/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理图片信息'
            	...
            	'Table_Header' => array(//4级
                	'ID', '背景图片', '添加时间', '图片2', '图片3',  '操作'
            	),
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zp/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zp/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,pic2,pic3')->where($aWhere),
            array($this,'abc')
         );
	}
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_zp1 pz on pz.id = o.pinzhong_id')
...
/* 		$list=M('Zp2')->select();//被添加内容的表
		...
		$this->$aaa('zp1',array(
				...
				array('type'=>'img','many'=>array(
						...
						array('title'=>"图片2",'type'=>"img",'name'=>"pic2",'value'=>'pic2','width'=>80,'height'=>97),
						array('title'=>"图片3",'type'=>"img",'name'=>"pic3",'value'=>'pic3','width'=>198,'height'=>100)
				)),
    ...
    public function add_pinzhong(){
    	...
    	$b=M('Zp1')->where(array('token'=>$token))->find();
    	if($b){
