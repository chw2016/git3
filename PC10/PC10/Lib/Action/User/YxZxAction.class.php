 ...
 *  技术支持：张银飞 ,李铭,张湘南
 **/
class YxZxAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
    ...
    public function index(){
        ...
        $this->table(
            array(
                ...
                'HeadHover' => U('YxZx/index', array('token' => $this->_sToken)),//栏目样式
                'Head_Opt' => array(
                    array(
                        'name'   => '添加市场资讯',//2级
                        'url'    => U('YxZx/add_content',array('token'=>$_SESSION['token']))
                    ),
                		
                		array(
                				'name'   => '市场资讯轮播图管理',//2级
                				'url'    => U('YxLb/index',array('type'=>'市场资讯'))
                		),
                ...
                'List_Opt' => array(
                    ...
                    /*               			array(
                                                    'name' => '其它信息',
                                                      'url'  => U('YxZx/xx',array('token'=>$_SESSION['token']))
                                            ), */

                    array(
                        'name' => '编辑',
                        'url'  => U('YxZx/save_content',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('YxZx/delete_content',array('token'=>$_SESSION['token']))
                    ),
                ),
             
            ),
            M('Yanxiang_zx')->count(),
            M('Yanxiang_zx')->field('id,title,pic,times')->order("times desc"),
       array($this,'abc')
        );
    }
    ...
    public function add_save($aaa){
        ...
        ),U('YxZx/index',array('token'=>$_SESSION['token'])),array($this,'bbc'));
