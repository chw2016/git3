 ...
 *  银波米业
 **/
class MruMtAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
	...
	//P($aWhere);die;
		$this->table(
        	array(
            	...
            	'HeadHover' => U('MruMt/index', array('token' => $this->_sToken)),
            	'Head_Opt' => array(
                 	array(
                    	'name'   => '添加',
                    	'url'    => U('MruMt/add_pinzhong')
                	) 
            	),
                'tips' => array(
                    '你可以在这里管理添加我要积分信息'
            	...
            	'Table_Header' => array(
                	'ID', '图片','红包使用记录','邀请好友购买提示信息','操作'
            	),
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('MruMt/PinzhongEdit')
                	),
                array(
	                    'name' => '删除',
	                    'url'  => U('MruMt/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,content,content2')->where($aWhere),
				array($this,'abc')
         );
	}
	...
	public function abc($data){
		foreach($data as $k=>$v){
			...
			$data[$k]['content']=str_substr($v['content'],20,'...');
			$data[$k]['content2']=str_substr($v['content2'],20,'...');
/* 			switch ($v['name']){
				case 1:$data[$k]['name']='减肥';break;
				case 2:$data[$k]['name']='美容';break;
				case 3:$data[$k]['name']='美体';break;
				case 4:$data[$k]['name']='养生';break;
				case 5:$data[$k]['name']='其它';break;
			} */
...
/*                 array(
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_mru_mt pz on pz.id = o.pinzhong_id')
    ...
    public function add_pinzhong(){
       $list=M('mru_mt')->where(array('token'=>$_SESSION['token']))->find();
       if($list) script("你已经添加过了");
        $this->add('mru_mt',array(
  
/*         	
       array('title'=>"我要积分名称",'type'=>"input",'name'=>"name",'value'=>'name'),
 */
		   array('type'=>'img','many'=>array(
                array('title'=>"图片",'type'=>"img",'name'=>"pic",'value'=>'pic','width'=>320,'height'=>101),
		   		
            )),
        		
        		array('title'=>"红包使用说明",'type'=>"textarea",'name'=>"content",'value'=>'content'),
        		array('title'=>"邀请好友购买提示信息",'type'=>"textarea_1",'name'=>"content2",'value'=>'content2'),
       

        ),U('MruMt/index'),array($this,'bbc'));
    ...
    public function PinzhongEdit(){
        ...
        $this->Edit('mru_mt',array(
               //array('type'=>'select','title'=>"我要积分名称",'name'=>"name",'value'=>'name','msg'=>'请填写标题咯','many'=>array(
        //	array('title'=>"我要积分名称",'type'=>"input",'name'=>"name",'value'=>'name'),
        ...
        ),U('MruMt/index', array('token' => $this->token)));
