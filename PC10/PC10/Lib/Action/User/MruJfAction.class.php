 ...
 *  银波米业
 **/
class MruJfAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
	...
	//P($aWhere);die;
		$this->table(
        	array(
            	...
            	'HeadHover' => U('MruJf/index', array('token' => $this->_sToken)),
            	'Head_Opt' => array(
     /*            	array(
                    	'name'   => '添加我要积分',
                    	'url'    => U('MruJf/add_pinzhong')
                	) */
            	),
                'tips' => array(
                    '你可以在这里管理添加我要积分信息'
            	...
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('MruJf/PinzhongEdit')
                	),
/* 	                array(
	                    'name' => '删除',
	                    'url'  => U('MruJf/PinzhongDel')
	                ), */
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,content')->where($aWhere),
				array($this,'abc')
         );
	}
...
/*                 array(
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_mru_jf2 pz on pz.id = o.pinzhong_id')
 ...
 */
    ...
    public function PinzhongEdit(){
        ...
        $this->Edit('mru_jf2',array(
               //array('type'=>'select','title'=>"我要积分名称",'name'=>"name",'value'=>'name','msg'=>'请填写标题咯','many'=>array(
        //	array('title'=>"我要积分名称",'type'=>"input",'name'=>"name",'value'=>'name'),

		   array('type'=>'img','many'=>array(
                array('title'=>"我要积分图片",'type'=>"img",'name'=>"pic",'value'=>'pic','width'=>'320','height'=>'101'),
            )),
            array('title'=>"积分获取方式",'type'=>"textarea",'name'=>"content",'value'=>'content'),

        ),U('MruJf/index', array('token' => $this->token)));
