 ...
 *  技术支持：张银飞 ,李铭,张湘南
 **/
class JdAdvCateAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
        ...
        $this->table(
            array(
                ...
                'Head_Opt' => array(
                    array(
                        'name'   => '添加顾问类型',//2级
                        'url'    => U('JdAdvCate/add_content',array('token'=>$_SESSION['token']))
                    ),
                ...
                'List_Opt' => array(
                    ...
                    array(
                        'name' => '编辑',
                        'url'  => U('JdAdvCate/save_content',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('JdAdvCate/delete_content',array('token'=>$_SESSION['token']))
                    ),
                ),
            ),
            M('jd_adv_cate')->where(array('token'=>$_SESSION['token']))->count(),
            M('jd_adv_cate')->field('id,name')->where(array('token'=>$_SESSION['token']))->order("sort")
        // array($this,'abc')
        );
    }
    ...
    public function add_save($aaa){
        //特殊点选框,复选框,下拉列表
        /* 		$list=M('JdAdvCate2')->select();//被添加内容的表
                foreach ($list as $k=>$v){
                    $list[$k]['content']=$v['name'];//把内容子段改成content
                    $list[$k]['value']=$v['id'];//把id子段改成value
                    unset($list[$k]['name']);//删除原来的内容子段
                } */
        $this->$aaa('jd_adv_cate',array(
            array('title'=>"顾问类型名",'type'=>"input",'name'=>"name",'value'=>'name','msg'=>'请填写顾问类型'),
        ),U('JdAdvCate/index',array('token'=>$_SESSION['token'])),array($this,'bbc'));
    ...
    public function delete_content(){
        //M('jd_adv_cate')->where(array('uid'=>$_GET['id']))->delete();//删除其它信息
        $this->del('jd_adv_cate');
    }
    //排序  只 能主健是id,子段是sort,才能排序 .有意见开发！
    public function sortajax(){
        $this->sortajaTable('jd_adv_cate');
    }
...
//其它信息开头
    public function xx(){
        $this->table(
            array(
                ...
                'HeadHover' => U('JdAdvCate/index', array('token' => $this->_sToken)),//栏目样式
                'Head_Opt' => array(
                    array(
                        'name'   => '返回',//2级
                        'url'    => U('JdAdvCate/index',array('uid'=>$_GET['id']))
                    )
                ),
                'tips' => array(//3级
                    '你可以在这里管理信息'
                ...
                'List_Opt' => array(
                    ...
                    array(
                        'name' => '查看',
                        'url'  => U('JdAdvCate/save_xx',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('JdAdvCate/delete_xx',array('token'=>$_SESSION['token']))
                    ),
                ),

            ),
            M('jd_adv_cate')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id']))->count(),
            M('jd_adv_cate')->field('id')->where(array('token'=>$_SESSION['token'],'uid'=>$_GET['id'])),
            array($this,'xxabc')
        );
    }
    ...
    public function add_save_xx($aaa){
        ...
        ),U('JdAdvCate/xx',array('token'=>$_SESSION['token'],'id'=>$_GET['uid'])),array($this,'bbc'),1);
