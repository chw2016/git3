 ...
 *  银波米业
 **/
class HuodongaAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
	...
	public function index(){
		...
		if($_GET['addid']){
			...
			//日志
			$list=M('mru_huodong')->where(array('id'=>$_GET['addid']))->find();
		...
		if($_GET['editid']){
			...
			//日志
			$list=M('mru_huodong')->where(array('id'=>$_GET['editid']))->find();
			if($list) M('mru_rz')->add(array(
					'name'=>$list['title'],
					'pic'=>$list['pic'],
					'content'=>$list['content'],
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Huodonga/index', array('token' => $_SESSION['token'])),
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加最新活动',
                    	'url'    => U('Huodonga/add_pinzhong',array('aid'=>$_GET['aid']))
                	)
            	),
                'tips' => array(
            	...
            	'Table_Header' => array(
                    'ID', '标题','是否开启','图片','发布时间','审核状态','操作'
            	),
            	'List_Opt' => array(
            			
/*             			array(
            					'name' => '报名信息',
            					'url'  => U('Huodonga/mb',array('aid'=>$_GET['aid']))
            			), */
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Huodonga/PinzhongEdit',array('aid'=>$_GET['aid']))
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Huodonga/PinzhongDel',array('aid'=>$_GET['aid']))
	                ),

                    array(
                        'name' => '置顶',
                        'url'  => U('Huodonga/zd',array('aid'=>$_GET['aid']))
                    ),
            	),
        	...
        	$this->pz->field('id,title,status,pic,add_time,state2')->order("add_time desc")->where($aWhere),
			array($this,'abc')
...
public function zd(){
    ...
    public function add_pinzhong(){
      ...
      // $_POST['aid']=$aid;
       ...
       /*      array('title'=>"经纬度",'type'=>"map",'lng'=>"position_x",'lat'=>'position_y') */
        ),U('Huodonga/index',array('aid'=>$_GET['aid'])),array($this,'bbc'));
    ...
    public function PinzhongEdit(){
        ...
        ),U('Huodonga/index', array('token' => $this->token,'aid'=>$_GET['aid'])));
    ...
    public function PinzhongDel(){
    	...
    	$id=$_GET['id'];
    	//print_r($id);die;
    	M('mru_huodong')->where(array('id'=>$id))->save(array('type'=>2,'state2'=>0));
    	$this->error2('需要公司总部审核,才能生效!');
    	...
    	$this->table(
    			array(
    					...
    					'Head_Opt' => array(
    							...
    							array(
    									'name'   => '返回 ',
    									'url'    => U('Huodonga/index',array('aid'=>$_GET['aid']))
    							)
    					),
    					'tips' => array(
    							'你可以在这里管理活动信息'
