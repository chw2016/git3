 ...
 *  银波米业1
 **/
class Dianphd2Action extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/* 		if($_GET['addid'] || $_GET['editid'] ){
			...
			//$this->success2('111',U('Dianphd2/index'));
		} */
...
/* 	    echo "<pre>";
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Dianphd2/index', array('token' => $this->_sToken,'aid'=>$aid)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加预约',//2级
                    	'url'    => U('Dianphd2/add_pinzhong',array('aid'=>$_GET['aid'],'modulename'=>$_GET['modulename'],'token'=>$_GET['token']))
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理品种信息'
...
/*             		array(
            					'name' => '查看预约信息',
            					'url'  => U('Dianphd2/ck',array('aid'=>$_GET['aid'],'modulename'=>$_GET['modulename'],'token'=>$_GET['token']))
            		), */
            		
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Dianphd2/PinzhongEdit',array('aid'=>$_GET['aid'],'modulename'=>$_GET['modulename'],'token'=>$_GET['token']))
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Dianphd2/PinzhongDel',array('aid'=>$_GET['aid'],'modulename'=>$_GET['modulename'],'token'=>$_GET['token']))
	                ),
            	)
        	), 
        	$this->pz->where(array('aid'=>$aid))->count(),
        	$this->pz->field('id,dname,pic,type,state,state2,audit,sort')->where(array('aid'=>$aid,'token'=>$_SESSION['token']))->order("sort"),
            array($this,'abc')
         );
	}
	...
	public function addEdit($aaa){
		//特殊点选框,复选框,下拉列表
/* 		$list=M('Dianphd22')->select();//被添加内容的表
		foreach ($list as $k=>$v){
		...
		$this->$aaa('mru_dianphd',array(
				...
				array('type'=>'checkbox','title'=>'预约每天开启时间','name'=>"hd_time",'value'=>'hd_time','msg'=>'请填写标题咯','many'=>array(
		...
		),U('Dianphd2/index',array('aid'=>$_GET['aid'],'modulename'=>$_GET['modulename'],'token'=>$_GET['token'])),array($this,'bbc'));
	}
...
/*         if(IS_POST){
   ...
   /**
    ...
    public function ck(){
    	...
    	$list=M('mru_wyyy')->field('id,name,type,time_xs,openid')->where($aWhere)->select();
    
    	foreach ($list as $ke => $v){
    		$list[$v['name']]['name']  = M('mru_dianphd')->where(array('id'=>$v['name']))->getField('dname');
    		$list[$v['name']]['value'] = $v['name']; 
    		unset($list[$ke]);
    	}
    	...
    	$this->table(
    			array(
    					'abc'=>123,
    					//'id' => 'id',//如果主键不是id，则需要设置
    					'HeadHover' => U('Dianphd2/ck', array('token' => $_SESSION['token'],'aid'=>$aid)),//栏目样式 
    					'Head_Opt' => array(
  /*   							array(
    									'name'   => '返回',//2级
    									'url'    => U('Dianphd2/index',array('token'=>$token,'aid'=>$aid,'modulename'=>$_GET['modulename']))
    							) */
    					),
    					'tips' => array(//3级
    							'你可以在这里管理信息'
    					...
    					'Table_Header' => array(//4级
    							'ID', '预约名称', '预约类型', '预约时间', '会员名称','手机','操作'
    					),
    					'List_Opt' => array(
    							array(
    									'name' => '查看留言',
    									'url'  => U('Dianphd2/ckEdit',array('token'=>$token,'aid'=>$aid,'pid'=>$_GET['id'],'modulename'=>$_GET['modulename']))
    							),
    							array(
    									'name' => '删除',
    									'url'  => U('Dianphd2/ckDel',array('token'=>$token,'aid'=>$aid,'modulename'=>$_GET['modulename']))
    							),
    							
    							array(
    									'name' => '该会员详细信息',
    									'url'  => U('Dianphd2/ckGr',array('token'=>$token,'aid'=>$aid,'uid'=>$_GET['id'],'modulename'=>$_GET['modulename'],'model'=>'mru_wyyy','url'=>'Dianphd2/ck'))
    							),
    					),
    			...
    			M('mru_wyyy')->field('id,name,type,time_xs,openid')->where($aWhere),
    			array($this,'ckabc')
    	);
    }
...
/*     			array('title'=>"预约名称",'type'=>"input",'name'=>"name",'value'=>'name'),
    	...
    	),U('Dianphd2/ck',array('aid'=>$_GET['aid'],'id'=>$_GET['pid'],'modulename'=>$_GET['modulename'])),array($this,'bbc'),1);
    }
