 ...
 *  银波米业1
 **/
class ShiAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/*                 array(
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Shi/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加导航市区',//2级
                    	'url'    => U('Shi/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理品种信息'
            	...
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Shi/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Shi/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,name,position_x,position_y')->where($aWhere)
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_mru_shi pz on pz.id = o.pinzhong_id')
	...
	public function addEdit($aaa){
		//print_r($_POST);die;
		//特殊点选框,复选框,下拉列表
/* 		$list=M('Shi2')->select();//被添加内容的表
		foreach ($list as $k=>$v){
			$list[$k]['content']=$v['name'];//把内容子段改成content
			$list[$k]['value']=$v['id'];//把id子段改成value
			unset($list[$k]['name']);//删除原来的内容子段
    	} */
		$this->$aaa('mru_shi',array(
				
				array('title'=>"市区名",'type'=>"input",'name'=>"name",'value'=>'name'),
			
				array('title'=>"经纬度",'type'=>"map",'name'=>'aname', 'lng'=>"position_x",'lat'=>'position_y'),
		),U('Shi/index'),array($this,'bbc'));
	}
