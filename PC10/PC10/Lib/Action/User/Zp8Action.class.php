 ...
 *  银波米业1
 **/
class Zp8Action extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
...
/*     			array(
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zp8/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加内容',//2级
                    	'url'    => U('Zp8/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理品种信息'
            	...
            	'Table_Header' => array(//4级
                	'ID', '背景图片', '中间图片', '文字1', '文字2', '文字3', '文字4','操作'
            	),
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zp8/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zp8/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,pic,pic2,name1,name2,name3,name4')->where($aWhere),
            array($this,'abc')
         );
	}
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_Zp8 pz on pz.id = o.pinzhong_id')
...
/* 		$list=M('Zp82')->select();//被添加内容的表
		...
		$this->$aaa('Zp8',array(
				...
				array('type'=>'img','many'=>array(
						...
						array('title'=>"中间图片",'type'=>"img",'name'=>"pic2",'value'=>'pic2','width'=>199,'height'=>139),

					
				)),
				
				array('title'=>"文字1",'type'=>"input",'name'=>"name1",'value'=>'name1'),
				array('title'=>"文字2",'type'=>"input",'name'=>"name2",'value'=>'name2'),
				array('title'=>"文字3",'type'=>"input",'name'=>"name3",'value'=>'name3'),
				array('title'=>"文字4",'type'=>"input",'name'=>"name4",'value'=>'name4'),
    ...
    public function add_pinzhong(){
    	...
    	$b=M('Zp8')->where(array('token'=>$token))->find();
    	if($b){
