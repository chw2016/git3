 ...
 *  技术支持：张银飞 ,李铭,张湘南
 **/
class YxRdAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
    ...
    public function index(){
        ...
        $this->table(
            array(
                ...
                'HeadHover' => U('YxRd/index', array('token' => $this->_sToken)),//栏目样式
                'Head_Opt' => array(
                    array(
                        'name'   => '添加研详动态',//2级
                        'url'    => U('YxRd/add_content',array('token'=>$_SESSION['token']))
                    ),
                ),
                'tips' => array(//3级
                    '你可以在这里管理信息'
                ...
                'List_Opt' => array(
                    ...
                    /*               			array(
                                                    'name' => '其它信息',
                                                      'url'  => U('YxRd/xx',array('token'=>$_SESSION['token']))
                                            ), */

                    array(
                        'name' => '编辑',
                        'url'  => U('YxRd/save_content',array('token'=>$_SESSION['token']))
                    ),
                    array(
                        'name' => '删除',
                        'url'  => U('YxRd/delete_content',array('token'=>$_SESSION['token']))
                    ),
                ),
             
            ),
            M('Yanxiang_rd')->count(),
            M('Yanxiang_rd')->field('id,title,pic,times')->order("times desc"),
       array($this,'abc')
        );
    }
    ...
    public function add_save($aaa){
        ...
        $this->$aaa('Yanxiang_rd',array(
        	...
        	array('title'=>"主题",'type'=>"textarea2",'name'=>"zt",'value'=>'zt','msg'=>'请填写input框'),
        	array('title'=>"图文详细",'type'=>"textarea",'name'=>"content",'value'=>'content'),
         
        ),U('YxRd/index',array('token'=>$_SESSION['token'])),array($this,'bbc'));
    ...
    public function delete_content(){
        //M('Yanxiang_rd')->where(array('uid'=>$_GET['id']))->delete();//删除其它信息
        $this->del('Yanxiang_rd');
    }
    //排序  只 能主健是id,子段是sort,才能排序 .有意见开发！
    public function sortajax(){
        $this->sortajaxTable('Yanxiang_rd');
    }
    //是否显示  只 能主健是id,子段是is_show 才能是否显示  有意见自己去开发！
    public function is_showAjax(){
    	$this->is_showAjaxTable('Yanxiang_rd');
    }
