 ...
 *  银波米业
 **/
class MruRzAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
    ...
    public function index(){
        ...
        $this->table(
            array(
                ...
                'HeadHover' => U('MruRz/index', array('token' => $this->_sToken)),
               'Head_Opt' => array(
/*                     array(
                        'name'   => '我要加盟',
                        'url'    => U('MruRz/index2')
     ...
     /*           		array(
            ...
            M('mru_rz')->where($aWhere)->count(),
            M('mru_rz')->field('id,aid,type,add_time')->order("add_time desc")->where($aWhere),
...
/*                     array(
                ...
                'List_Opt' => array(
                    array(
                        'name' => '查看详情',
                        'url'  => U('MruRz/jlk',array('uid'=>$_GET['id']))
                    ),
                     array(
                        'name' => '删除',
                        'url'  => U('MruRz/jlk2')
	...
	public function dianpuAdd(){
		if(IS_POST){
            ...
            $this->assign(array(
                ...
                'FormUrl' => U('MruRz/dianpuAdd',array('token'=>$_SESSION['token']))
            ));
	...
	public function dianpuEdit(){
		...
		if(IS_POST){
			...
			$this->assign(array(
                'info' => $Item,
                'id' => $iID,
                'FormUrl' => U('MruRz/dianpuEdit',array('token'=>$_SESSION['token'])),
                'ExtraBtn' => array(
                    array(
                        'url'  => U('MruRz/dianpu', array('token' => $this->_sToken)),
                        'name' => '返回'
                )),
            ));
            $this->UDisplay('dianpu_add');
	...
	public function order(){
        ...
        $this->assign(array(
            //'id' => 'id',//如果主键不是id，则需要设置
            'HeadHover' => U('MruRz/order', array('token' => $this->_sToken)),
            'Head_Opt' => array(),
            'Table_Header' => array(
                'ID', '品种', '手机号', '店铺名',
        		'地址', '描述','状态', '下单时间',
            ...
            'List_Opt' => array(
                array(
                    'name' => '查看详细',
                    'url'  => U('MruRz/OrderShow', array('token' => $this->_sToken))
                ),
                array(
                    'name' => '完成',
                    'url'  => U('MruRz/orderHandle', array(
                    	'token' => $this->_sToken,
    ...
    public function PinzhongEdit(){
        ...
        ),U('MruRz/index', array('token' => $this->token)));
    ...
    public function ck(){
     	$id=$_GET['id'];
     	$list=M('mru_rz')->where(array('id'=>$id))->find();
    ...
    public function ck2(){
    	$id=$_GET['id'];
    	$list=M('mru_rz')->where(array('id'=>$id))->delete();
    	if($list){
    		$this->error2("删除成功");
    	}else{
    		$this->error2("删除失败");
...
/*     							array(
    ...
    public function savejm(){
    	...
    	),U('MruRz/addjm', array('token' => $this->token)));
    
    }
    
    //排序  只 能主健是id,子段是sort 才能排序  有意见自己去开发！
    public function sortajax(){
    	$this->sortajaxTable('mru_rz');
    }
