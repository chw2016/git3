 ...
 *  银波米业1
 **/
class ZjjsAction extends TableAction {
    /**
     *  定义项目模板BaseDir
     **/
	...
	public function index(){
		...
		$this->table(
        	array(
            	...
            	'HeadHover' => U('Zjjs/index', array('token' => $this->_sToken)),//栏目样式 
            	'Head_Opt' => array(
                	array(
                    	'name'   => '添加专家',//2级
                    	'url'    => U('Zjjs/add_pinzhong')
                	)
            	),
                'tips' => array(//3级
                    '你可以在这里管理专家信息'
            	...
            	'Table_Header' => array(//4级
                	'ID', '专家名称', '所在地', '专家头像','排序', '操作'
            	),
            	'List_Opt' => array(
                	array(
                    	'name' => '编辑',
                    	'url'  => U('Zjjs/PinzhongEdit')
                	),
	                array(
	                    'name' => '删除',
	                    'url'  => U('Zjjs/PinzhongDel')
	                ),
            	)
        	), 
        	$this->pz->where($aWhere)->count(),
        	$this->pz->field('id,name,dz,pic,sort')->order("sort")->where($aWhere),
            array($this,'abc')
         );
	}
	...
	public function abc($data){
		foreach($data as $k=>$v){
			...
			//$data[$k]['content']=substr($v['content'],0,78); 
	...
	public function order(){
		...
		$aInfo  = $Model->table('tp_miye_order o')
			...
			->join('tp_mru_zjjs pz on pz.id = o.pinzhong_id')
	...
	public function addEdit($aaa){
		//特殊点选框,复选框,下拉列表
/* 		$list=M('Zjjs2')->select();//被添加内容的表
		foreach ($list as $k=>$v){
			$list[$k]['content']=$v['name'];//把内容子段改成content
			$list[$k]['value']=$v['id'];//把id子段改成value
			unset($list[$k]['name']);//删除原来的内容子段
    	} */
		$this->$aaa('mru_zjjs',array(
				
				array('title'=>"专家名称",'type'=>"input",'name'=>"name",'value'=>'name'),
				array('title'=>"所在地",'type'=>"input",'name'=>"dz",'value'=>'name'),

			
			
				
				array('type'=>'img','many'=>array(
						array('title'=>"专家头像",'type'=>"img",'name'=>"pic",'value'=>'pic'),
						
				)),
			
				array('title'=>"专家简介",'type'=>"textarea",'name'=>"content",'value'=>'content'),
				
		),U('Zjjs/index'),array($this,'bbc'));
	}
